...
class FileUploadService {
    ...
    async uploadFile(data) {
        ...
        const { resource, workspace, participants } = await this.validateResourceAccess(attachedTo, attachedToId, uploadedBy);
        ...
        await this.emitFileUploadEvent(fileAttachment, attachedTo, resource, workspace, participants, uploadedBy);
        return fileAttachment;
    }
    /**
     * Validate resource access and get participants
     */
    async validateResourceAccess(attachedTo, attachedToId, userId) {
        let resource;
        let workspace;
        let participants = [];
        switch (attachedTo) {
            ...
            case "TaskComment":
                ...
                const isCommentMember = workspace.members.some((m) => m.user.toString() === userId);
                if (!isCommentMember) {
     ...
     */
    async emitFileUploadEvent(fileAttachment, attachedTo, resource, workspace, participants, uploadedBy) {
        const eventData = {
            file: {
            ...
            if (participantId !== uploadedBy) {
                ...
                if (isOnline) {
                    ...
                    notificationService
                        ...
                        .catch((error) => {
                        console.error(`[FileUpload] Failed to send notification to ${participantId}:`, error);
                    });
                }
            }
        });
     ...
     */
    async getFiles(attachedTo, attachedToId, userId, options = {}) {
        // Validate access
    ...
    async deleteFile(fileId, userId) {
        ...
        // Validate access
        const { workspace } = await this.validateResourceAccess(file.attachedTo, file.attachedToId.toString(), userId);
