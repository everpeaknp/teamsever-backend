...
class ActivityService {
    ...
    async createComment(data) {
        ...
        try {
            emitTaskEvent(taskId, "comment_added", {
        ...
        if (mentions.length > 0) {
            try {
                await enhancedNotificationService.notifyMentions(taskId, userId, mentions, content);
            }
            catch (error) {
        ...
        if (task.assignee && task.assignee.toString() !== userId) {
            try {
                await enhancedNotificationService.notifyTaskComment(taskId, userId, content);
            }
            catch (error) {
    ...
    async createUpdate(data) {
        const { taskId, userId, fieldChanged, oldValue, newValue, isSystemGenerated = false, } = data;
        // Verify task exists
        const task = await Task.findOne({
            _id: taskId,
            isDeleted: false,
        ...
        // Emit real-time event
        try {
            emitTaskEvent(taskId, "activity_added", {
     ...
     */
    async getTaskActivity(taskId, userId, options = {}) {
     ...
     * Update a comment
     */
    async updateComment(activityId, userId, content) {
        ...
        // Emit real-time event
        try {
            emitTaskEvent(activity.task.toString(), "comment_updated", {
                activity: populatedActivity,
            }, userId);
        }
        catch (error) {
     ...
     * Delete a comment
     */
    async deleteComment(activityId, userId) {
        const activity = await Activity.findOne({
            _id: activityId,
            type: "comment",
            isDeleted: false,
        ...
        // Emit real-time event
        try {
            emitTaskEvent(activity.task.toString(), "comment_deleted", {
     ...
     */
    async addReaction(activityId, userId, emoji) {
        const activity = await Activity.findOne({
            _id: activityId,
            type: "comment",
            isDeleted: false,
        ...
        // Emit real-time event
        try {
            emitTaskEvent(activity.task.toString(), "comment_reaction", {
                activityId: activity._id.toString(),
                userId,
                emoji,
            }, userId);
    ...
    async removeReaction(activityId, userId) {
        ...
        // Emit real-time event
        try {
            emitTaskEvent(activity.task.toString(), "comment_reaction_removed", {
                activityId: activity._id.toString(),
                userId,
            }, userId);
        }
