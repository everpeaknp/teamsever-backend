...
class CommentService {
    ...
    async createComment(data) {
        ...
        try {
            await enhancedNotificationService.notifyCommentAdded(comment._id.toString(), taskId, userId, content, mentions);
        }
        catch (error) {
     ...
     */
    async getComments(taskId, userId, options = {}) {
        // Validate task access
    ...
    async editComment(userId, commentId, content) {
        ...
        // Validate task access
        const { task, workspace } = await this.validateTaskAccess(comment.task.toString(), userId);
        ...
        try {
            emitTaskEvent(comment.task.toString(), "comment_updated", {
                comment: {
                    _id: comment._id,
                    content: comment.content,
                    edited: comment.edited,
        ...
        // Send notifications using enhanced notification service
        try {
            await enhancedNotificationService.notifyCommentUpdated(comment._id.toString(), comment.task.toString(), userId);
        }
        catch (error) {
            console.error("[Comment] Failed to send notifications:", error);
        }
    ...
    async deleteComment(userId, commentId) {
        ...
        // Emit real-time event
        try {
            emitTaskEvent(comment.task.toString(), "comment_deleted", {
        ...
        // Send notifications using enhanced notification service
        try {
            await enhancedNotificationService.notifyCommentDeleted(comment._id.toString(), comment.task.toString(), userId);
        }
        catch (error) {
            console.error("[Comment] Failed to send notifications:", error);
        }
