...
const { roleHasPermission, spacePermissionHasAction, folderPermissionHasAction, listPermissionHasAction } = require("./permission.constants");
...
class PermissionService {
     ...
     */
    async can(userId, action, context) {
        try {
            console.log('\n========================================');
            ...
            console.log('  ➜ WorkspaceRole.ADMIN value:', JSON.stringify(permission_types_1.WorkspaceRole.ADMIN));
            console.log('  ➜ Strict equality (===):', workspaceRole === permission_types_1.WorkspaceRole.ADMIN);
            console.log('  ➜ Loose equality (==):', workspaceRole == permission_types_1.WorkspaceRole.ADMIN);
            console.log('  ➜ String comparison:', String(workspaceRole) === String(permission_types_1.WorkspaceRole.ADMIN));
            if (workspaceRole === permission_types_1.WorkspaceRole.ADMIN) {
            ...
            if (context.listId) {
                const listPermission = await this.getListPermissionLevel(userId, context.listId);
                if (listPermission) {
            ...
            if (context.folderId) {
                const folderPermission = await this.getFolderPermissionLevel(userId, context.folderId);
                if (folderPermission) {
            ...
            if (context.spaceId) {
                const spacePermission = await this.getSpacePermissionLevel(userId, context.spaceId);
                if (spacePermission) {
                ...
                if (!context.listId &&
                    ...
                    workspaceRole === permission_types_1.WorkspaceRole.MEMBER) {
            ...
            if (!hasWorkspacePermission) {
                ...
                if (this.isTaskAction(action) && context.resourceType === "task") {
                    ...
                    const result = await this.checkTaskPermission(userId, action, context, workspaceRole);
    ...
    async getUserRole(userId, workspaceId) {
        try {
            ...
            if (workspace.owner.toString() === userId) {
                ...
                return permission_types_1.WorkspaceRole.OWNER;
            }
     ...
     */
    async getSpacePermissionLevel(userId, spaceId) {
        try {
     ...
     */
    async getFolderPermissionLevel(userId, folderId) {
        try {
     ...
     */
    async getListPermissionLevel(userId, listId) {
        try {
     ...
     */
    async getTablePermissionLevel(userId, tableId) {
        try {
     ...
     * @returns Promise<boolean>
     */
    async canAccessTable(userId, tableId, workspaceId) {
        try {
     ...
     * @returns Promise<boolean>
     */
    async isOwner(userId, workspaceId) {
        try {
     ...
     * @returns Promise<boolean>
     */
    async isAdminOrOwner(userId, workspaceId) {
        const role = await this.getUserRole(userId, workspaceId);
        return role === permission_types_1.WorkspaceRole.ADMIN || role === permission_types_1.WorkspaceRole.OWNER;
    }
    /**
     ...
     */
    async checkTaskPermission(userId, action, context, role) {
        // Admins and owners can do anything
        if (role === permission_types_1.WorkspaceRole.ADMIN || role === permission_types_1.WorkspaceRole.OWNER) {
            return true;
        }
     ...
     * @returns Promise<boolean>
     */
    async isSpaceMember(userId, spaceId) {
        try {
    ...
    normalizeRole(role) {
        ...
        let result;
        if (roleLower === "owner") {
            result = permission_types_1.WorkspaceRole.OWNER;
            console.log('    ➜ Matched: OWNER');
        }
        else if (roleLower === "admin") {
            result = permission_types_1.WorkspaceRole.ADMIN;
            console.log('    ➜ Matched: ADMIN');
        }
        else if (roleLower === "member") {
            result = permission_types_1.WorkspaceRole.MEMBER;
            console.log('    ➜ Matched: MEMBER');
        }
        else if (roleLower === "guest") {
            result = permission_types_1.WorkspaceRole.GUEST;
        ...
        console.log('    ➜ WorkspaceRole.ADMIN constant:', permission_types_1.WorkspaceRole.ADMIN);
        console.log('    ➜ Are they equal?:', result === permission_types_1.WorkspaceRole.ADMIN);
        return result;
    }
}
