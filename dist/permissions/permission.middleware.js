    ...
    return async (req, res, next) => {
        try {
            ...
            if (!hasPermission) {
                ...
                return next(new AppError(`You do not have permission to perform this action: ${action}`, 403));
            }
 ...
 */
async function getFolderId(req) {
...
async function getWorkspaceId(req) {
    ...
    if (folderId) {
        ...
        if (folder && folder.spaceId) {
            ...
            if (typeof folder.spaceId === 'object' && folder.spaceId.workspace) {
                return folder.spaceId.workspace.toString();
            }
...
function getResourceType(req) {
    ...
    if (req.baseUrl.includes("/spaces"))
        return "space";
    if (req.baseUrl.includes("/folders"))
        return "folder";
    if (req.baseUrl.includes("/lists"))
        return "list";
