...
attachmentSchema.virtual("attachmentType").get(function () {
    if (this.task)
        return "task";
    if (this.comment)
        return "comment";
...
attachmentSchema.statics.getByTask = function (taskId) {
    return this.find({ task: taskId, isDeleted: false }).populate("uploadedBy", "name email");
};
attachmentSchema.statics.getByComment = function (commentId) {
    return this.find({ comment: commentId, isDeleted: false }).populate("uploadedBy", "name email");
};
attachmentSchema.statics.getByConversation = function (conversationId) {
    return this.find({ conversation: conversationId, isDeleted: false }).populate("uploadedBy", "name email");
};
attachmentSchema.statics.getByMessage = function (messageId) {
    return this.find({ message: messageId, isDeleted: false }).populate("uploadedBy", "name email");
};
