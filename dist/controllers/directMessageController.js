...
const startConversation = asyncHandler(async (req, res) => {
    ...
    const conversation = await directMessageService.startConversation(currentUserId, targetUserId);
    res.status(200).json({
        success: true,
        data: conversation,
...
const getConversation = asyncHandler(async (req, res) => {
    const userId = req.user.id;
    const { conversationId } = req.params;
    const conversation = await directMessageService.getConversationById(conversationId, userId);
    res.status(200).json({
        success: true,
        data: conversation,
    });
