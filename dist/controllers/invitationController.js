...
// @access  Private (Admin/Owner only)
const getWorkspaceInvitations = asyncHandler(async (req, res, next) => {
    const { workspaceId } = req.params;
...
// @access  Private (Admin/Owner only)
const cancelInvitation = asyncHandler(async (req, res, next) => {
    const { workspaceId, invitationId } = req.params;
    const result = await invitationService.cancelInvitation(invitationId, req.user.id, workspaceId);
    res.status(200).json({
        success: true,
        data: result
    });
...
// @access  Private
const getMyInvitations = asyncHandler(async (req, res, next) => {
