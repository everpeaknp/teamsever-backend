...
const getAttendanceReport = asyncHandler(async (req, res) => {
    ...
    const data = await attendanceService.getAttendanceReport(workspaceId, req.user.id, { startDate, endDate, userId, projectId });
    res.status(200).json({
        success: true,
        data
    });
...
const exportAttendanceCSV = asyncHandler(async (req, res) => {
    const { workspaceId } = req.params;
    const { startDate, endDate, userId, projectId } = req.query;
    const csvData = await attendanceService.exportAttendanceCSV(workspaceId, req.user.id, { startDate, endDate, userId, projectId });
...
const exportAttendanceExcel = asyncHandler(async (req, res) => {
    const { workspaceId } = req.params;
    const { startDate, endDate, userId, projectId } = req.query;
    const excelBuffer = await attendanceService.exportAttendanceExcel(workspaceId, req.user.id, { startDate, endDate, userId, projectId });
