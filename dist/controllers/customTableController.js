...
async function checkTablePermission(tableId, userId, requiredLevel) {
...
const createTable = asyncHandler(async (req, res, next) => {
    ...
    const table = await customTableService.createTable(spaceId, name, req.user.id, columns, initialRows);
    res.status(201).json({
        success: true,
        data: table
    });
...
// @access  Private
const getSpaceTables = asyncHandler(async (req, res, next) => {
    const { spaceId } = req.params;
    ...
    if (isAdminOrOwner) {
        ...
        accessibleTables = allTables.map((table) => ({
            ...table.toObject(),
            userPermission: 'FULL'
        }));
    }
        ...
        const permissionMap = new Map(userTablePermissions.map((tm) => [tm.table.toString(), tm.permissionLevel]));
        accessibleTables = allTables
            .filter((table) => permissionMap.has(table._id.toString()))
            .map((table) => ({
            ...table.toObject(),
...
const addColumn = asyncHandler(async (req, res, next) => {
    const { tableId } = req.params;
...
// @access  Private
const updateColumn = asyncHandler(async (req, res, next) => {
...
// @access  Private (Requires FULL permission)
const addRow = asyncHandler(async (req, res, next) => {
    const { tableId } = req.params;
...
const updateCell = asyncHandler(async (req, res, next) => {
...
// @access  Private (Requires EDIT or FULL permission)
const updateCellColor = asyncHandler(async (req, res, next) => {
    const { tableId, rowId, columnId } = req.params;
...
// @access  Private (Requires EDIT or FULL permission)
const updateCellTextColor = asyncHandler(async (req, res, next) => {
    const { tableId, rowId, columnId } = req.params;
    const { color } = req.body;
    // Validate hex color format if color is provided
    if (color !== null && color !== undefined) {
...
// @access  Private
const exportTable = asyncHandler(async (req, res, next) => {
    const { tableId } = req.params;
