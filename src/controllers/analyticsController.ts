 ...
 */
exports.getVelocity = async (req: any, res: any, next: any) => {
  try {
    ...
    const velocityData = await analyticsService.getVelocity(
      workspaceId,
      userId,
      days
    );
 ...
 */
exports.getLeadTime = async (req: any, res: any, next: any) => {
  try {
    const { workspaceId } = req.query;
    const userId = req.user.id;
    const days = parseInt(req.query.days as string) || 30;
    ...
    const leadTimeData = await analyticsService.getLeadTime(
      workspaceId,
      userId,
      days
    );
 ...
 */
exports.getWorkspaceAnalytics = async (req: any, res: any, next: any) => {
  try {
 ...
 */
exports.getWorkspaceOverview = async (req: any, res: any, next: any) => {
  try {
    const { workspaceId } = req.params;
    const userId = req.user.id;
 ...
 */
exports.getTeamWorkload = async (req: any, res: any, next: any) => {
  try {
    const { workspaceId } = req.params;
    const userId = req.user.id;

    const workload = await analyticsService.getTeamWorkload(
      workspaceId,
      userId
    );
