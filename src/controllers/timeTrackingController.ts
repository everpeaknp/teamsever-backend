 ...
 * @access  Private
 */
const startTimer = asyncHandler(async (req: any, res: any) => {
  const { id } = req.params;
  ...
  if (activeTask) {
    throw new AppError(
      `You already have an active timer on task: ${activeTask.title}. Please stop it first.`,
      400
    );
  }
 ...
 * @access  Private
 */
const stopTimer = asyncHandler(async (req: any, res: any) => {
  const { id } = req.params;
  const userId = req.user.id;
 ...
 * @access  Private
 */
const getActiveTimer = asyncHandler(async (req: any, res: any) => {
  const userId = req.user.id;
 ...
 * @access  Private
 */
const addManualTimeLog = asyncHandler(async (req: any, res: any) => {
  const { id } = req.params;
  const userId = req.user.id;
 ...
 * @access  Private
 */
const getTimeLogs = asyncHandler(async (req: any, res: any) => {
  const { id } = req.params;
  ...
  const logs = task.timeLogs.map((log: any) => ({
    ...log.toObject(),
 ...
 * @access  Private
 */
const deleteTimeLog = asyncHandler(async (req: any, res: any) => {
  ...
  const logIndex = task.timeLogs.findIndex(
