...
const getWorkspaceActiveTimers = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { workspaceId } = req.params;

    const data = await timeEntryService.getWorkspaceActiveTimers(
      workspaceId,
      req.user!.id
    );
...
// @access  Private (Admin/Owner only)
const getTeamTimesheets = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { workspaceId } = req.params;
    const { startDate, endDate, userId, projectId } = req.query;

    const data = await timeEntryService.getTeamTimesheets(
      workspaceId,
      req.user!.id,
      {
...
// @access  Private (Admin/Owner only)
const adminStopTimer = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { entryId } = req.params;
    const { reason } = req.body;

    const timeEntry = await timeEntryService.adminStopTimer(
      entryId,
      req.user!.id,
      reason
    );
...
// @access  Private (Admin/Owner only)
const getWorkspaceTimeStats = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { workspaceId } = req.params;

    const stats = await timeEntryService.getWorkspaceTimeStats(
      workspaceId,
      req.user!.id
    );
...
// @access  Private (Admin/Owner only)
const cleanupOrphanedTimers = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { workspaceId } = req.params;

    const result = await timeEntryService.cleanupOrphanedTimers(
      workspaceId,
      req.user!.id
    );
...
// @access  Private (Admin/Owner only)
const stopAllUserTimers = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { workspaceId, userId } = req.params;
    const { reason } = req.body;

    const result = await timeEntryService.stopAllUserTimers(
      workspaceId,
      userId,
      req.user!.id,
      reason
