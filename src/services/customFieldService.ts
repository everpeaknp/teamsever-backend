...
class CustomFieldService {
   ...
   */
  private async checkAdminOrOwner(workspaceId: string, projectId: string | undefined, userId: string) {
   ...
   */
  async getFieldsByWorkspace(workspaceId: string, userId: string) {
   ...
   */
  async getFieldsByProject(projectId: string, userId: string) {
  ...
  async updateCustomField(
    ...
    userId: string,
    updateData: UpdateCustomFieldData
  ) {
    ...
    // Verify permissions
    await this.checkAdminOrOwner(
      customField.workspace.toString(),
      customField.project?.toString(),
      userId
    );

    // Update fields
    if (updateData.name) customField.name = updateData.name;
   ...
   */
  async deleteCustomField(customFieldId: string, userId: string) {
    const customField = await CustomField.findOne({
      _id: customFieldId,
      isDeleted: false
    });
   ...
   */
  validateFieldValue(type: string, value: any, options?: string[]): any {
    switch (type) {
      case "text":
  ...
  async validateCustomFieldValues(
    ...
    for (const fieldValue of customFieldValues) {
      ...
      const validatedValue = this.validateFieldValue(
        customField.type,
        fieldValue.value,
        customField.options
      );
