...
class TimeEntryService {
   ...
   */
  async getWorkspaceActiveTimers(workspaceId: string, adminId: string) {
  ...
  async getTeamTimesheets(
    ...
    const workspaceTotal = userSummary.reduce(
      (sum: number, item: any) => sum + item.totalDuration,
      0
    );

    // Format durations
    userSummary.forEach((item: any) => {
      item.totalDurationFormatted = this.formatDuration(item.totalDuration);
    });

    projectSummary.forEach((item: any) => {
      item.totalDurationFormatted = this.formatDuration(item.totalDuration);
    });
    ...
    if (filters.userId) {
      ...
      detailedEntries = detailedEntries.map((entry: any) => ({
        ...entry,
   ...
   */
  async adminStopTimer(entryId: string, adminId: string, reason?: string) {
    const timeEntry = await TimeEntry.findOne({
      _id: entryId,
      isDeleted: false
    });
   ...
   */
  async getWorkspaceTimeStats(workspaceId: string, adminId: string) {
    // Verify admin has access
    const workspace = await Workspace.findOne({
      _id: workspaceId,
      isDeleted: false
   ...
   */
  async cleanupOrphanedTimers(workspaceId: string, adminId: string) {
    // Verify admin has access
    const workspace = await Workspace.findOne({
      _id: workspaceId,
      isDeleted: false
   ...
   */
  async stopAllUserTimers(workspaceId: string, userId: string, adminId: string, reason?: string) {
    // Verify admin has access
    const workspace = await Workspace.findOne({
      _id: workspaceId,
      isDeleted: false
