...
class AttachmentService {
   ...
   */
  async deleteTaskAttachments(taskId: string) {
   ...
   * Validate file metadata
   */
  private validateFileMetadata(metadata: AttachmentMetadata) {
    ...
    if (!fileType || !uploadConfig.allowedMimeTypes.includes(fileType)) {
      throw new AppError(
        `File type not allowed. Allowed types: ${uploadConfig.allowedMimeTypes.join(", ")}`,
        400
      );
    }
    ...
    if (!uploadConfig.allowedExtensions.includes(ext)) {
      throw new AppError(
        `File extension not allowed. Allowed extensions: ${uploadConfig.allowedExtensions.join(", ")}`,
        400
      );
    }
   ...
   */
  private async deleteFromCloudinary(publicId: string, resourceType: string = "auto") {
    try {
