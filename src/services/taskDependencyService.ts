...
class TaskDependencyService {
   ...
   */
  async createDependency(data: CreateDependencyData) {
   ...
   */
  async deleteDependency(dependencyId: string, userId: string) {
   ...
   */
  async getBlockingTasks(taskId: string, userId: string) {
    // Verify task exists
    const task = await Task.findOne({
      _id: taskId,
      isDeleted: false
    ...
    for (const dep of dependencies) {
      const dependsOnTask: any = dep.dependsOn;
   ...
   */
  private getBlockingReason(type: string, dependsOnStatus: string): string {
    switch (type) {
      case "FS":
