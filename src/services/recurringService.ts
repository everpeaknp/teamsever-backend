...
class RecurringService {
   ...
   */
  async processRecurringTasks(): Promise<RecurringTaskResult> {
   ...
   */
  private async cloneTask(originalTask: any): Promise<any> {
    const taskData: any = {
   ...
   */
  private async updateNextOccurrence(task: any): Promise<void> {
    const nextDate = this.calculateNextOccurrence(
      task.nextOccurrence || new Date(),
      task.frequency,
      task.interval || 1
    );
   ...
   */
  private calculateNewDueDate(task: any): Date | undefined {
    if (!task.dueDate) {
      return undefined;
    }
   ...
   */
  async getRecurringTasks(workspaceId: string, userId: string): Promise<any[]> {
   ...
   */
  async getRecurringTaskInstances(recurringTaskId: string, userId: string): Promise<any[]> {
   ...
   */
  async stopRecurringTask(taskId: string, userId: string): Promise<any> {
