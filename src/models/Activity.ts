...
activitySchema.virtual("changeDescription").get(function (this: any) {
  if (this.type !== "update") return null;
...
activitySchema.methods.formatValue = function (value: any, field: string) {
...
activitySchema.methods.softDelete = async function (userId: string) {
  this.isDeleted = true;
...
activitySchema.methods.addReaction = async function (userId: string, emoji: string) {
...
activitySchema.methods.removeReaction = async function (userId: string) {
  this.reactions = this.reactions.filter(
    (r: any) => r.user.toString() !== userId
  );
  return await this.save();
...
activitySchema.statics.getTaskActivity = function (
  taskId: string,
...
activitySchema.statics.getUserActivity = function (
  userId: string,
  workspaceId: string,
...
activitySchema.statics.createUpdateActivity = async function (
  taskId: string,
  userId: string,
  workspaceId: string,
  fieldChanged: string,
...
activitySchema.statics.createCommentActivity = async function (
  taskId: string,
  userId: string,
  workspaceId: string,
  content: string,
